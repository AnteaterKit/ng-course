...
@Component({
  ...
  template: `
      <span class='input-field-wrapper'>
          ...
          <span *ngIf='shAddonSuffix' class='input-field-addOn input-field-addOn-suffix'>
              <ng-template [ngTemplateOutlet]="shAddonSuffix"></ng-template>
          </span>
      </span>
  `,
...
export class InputFieldComponent implements OnInit, OnChanges, OnDestroy  {
  ...
  @Input() shSuffix?: TemplateRef<any>;
  @Input() shAddonSuffix?: TemplateRef<any>;
  @Input() active = false;
  ...
  ngOnDestroy(): void {
    // генерируем событие для монитора cdk
    this.destroy$.next();
    // завершаем subject.
